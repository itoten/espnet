# emns speech emotion recognition (single-corpus baseline)

Trains `wavlmasp` (fine-tuned WavLM Large + attentive statistics pooling +
FC head) on emns alone, and scores it on emns's own test split.
Labels are emns's native set (8 classes), the same
set `egs3/multicorpus_full20/wavlmasp` uses for emns's test scoring, so
the two are directly comparable: same labels, same test data, the only
difference is whether training data came from emns alone or from the
ten-corpus merge. See `espnet_work/per_corpus_baseline_plan.md`.

`max_epochs` (131) and `EarlyStopping.patience` (15) are
...
`EMNS_OUTPUT` must already point at emns's own prepared
data (same as `egs3/emns/esp2_cls`); `EMNS_WAVLMASP_OUTPUT` controls where this
