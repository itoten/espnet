# cremad speech emotion recognition (single-corpus baseline)

Trains `wavlmasp` (fine-tuned WavLM Large + attentive statistics pooling +
FC head) on cremad alone, and scores it on cremad's own test split.
Labels are cremad's native set (6 classes), the same
set `egs3/multicorpus_full20/wavlmasp` uses for cremad's test scoring, so
the two are directly comparable: same labels, same test data, the only
difference is whether training data came from cremad alone or from the
...
`CREMAD_OUTPUT` must already point at cremad's own prepared
data (same as `egs3/cremad/esp2_cls`); `CREMAD_WAVLMASP_OUTPUT` controls where this
