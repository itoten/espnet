# enterface05 speech emotion recognition (single-corpus baseline)

Trains `wavlmasp` (fine-tuned WavLM Large + attentive statistics pooling +
FC head) on enterface05 alone, and scores it on enterface05's own test split.
Labels are enterface05's native set (6 classes), the same
set `egs3/multicorpus_full20/wavlmasp` uses for enterface05's test scoring, so
the two are directly comparable: same labels, same test data, the only
difference is whether training data came from enterface05 alone or from the
ten-corpus merge. See `espnet_work/per_corpus_baseline_plan.md`.

`max_epochs` (100) and `EarlyStopping.patience` (15) are
...
`ENTERFACE05_OUTPUT` must already point at enterface05's own prepared
data (same as `egs3/enterface05/esp2_cls`); `ENTERFACE05_WAVLMASP_OUTPUT` controls where this
