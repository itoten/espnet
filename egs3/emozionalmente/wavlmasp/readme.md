# emozionalmente speech emotion recognition (single-corpus baseline)

Trains `wavlmasp` (fine-tuned WavLM Large + attentive statistics pooling +
FC head) on emozionalmente alone, and scores it on emozionalmente's own test split.
Labels are emozionalmente's native set (7 classes), the same
set `egs3/multicorpus_full20/wavlmasp` uses for emozionalmente's test scoring, so
the two are directly comparable: same labels, same test data, the only
difference is whether training data came from emozionalmente alone or from the
...
`EMOZIONALMENTE_OUTPUT` must already point at emozionalmente's own prepared
data (same as `egs3/emozionalmente/esp2_cls`); `EMOZIONALMENTE_WAVLMASP_OUTPUT` controls where this
