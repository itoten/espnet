"""Area under the ROC curve metric for classification."""
...
from espnet3.systems.wavlmasp.metrics.scoring_utils import (
    build_score_matrix,
    build_target_matrix,
    load_class_labels,
    require_sklearn,
...
class AUC(BaseMetric):
    ...
    The area under the ROC curve is computed per class and then macro
    averaged, matching the ``mean_auc`` column of espnet2's ``cls_score.py``.
    ...
    .. code-block:: yaml
        ...
        - metric:
            _target_: espnet3.systems.wavlmasp.metrics.auc.AUC
    ...
    def __call__(
        ...
        """Compute the macro-averaged one-vs-rest AUC.
